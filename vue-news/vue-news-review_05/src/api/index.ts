...
function fetchUser(id: string): AxiosPromise<UserInfo> {
  const url = `${api.user}${id}.json`;
  return axios.get(url);
}

function fetchItem(id: string): AxiosPromise<ItemInfo> {
  const url = `${api.item}${id}.json`;
  return axios.get(url);
}

function fetchList(type: string): AxiosPromise<ListInfo[]> {
